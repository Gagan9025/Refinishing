...
async function loadBatchDashboard() {
  ...
  try {
    const res = await API.request(`/api/batch/data?date=${selectedDate}`);
    currentData = res.data;
    renderBatchDashboard();
  } catch (err) {
    showToast(err.message || 'Error loading batch data', 'error');
  }
}
...
async function saveTarget() {
  ...
  try {
    const res = await API.request('/api/batch/target', {
      method: 'POST',
      body: JSON.stringify({ date: dateVal, dailyTarget: targetVal })
    });

    showToast('Batch daily target saved successfully!', 'success');
    currentData = res.data;
    renderBatchDashboard();
  } catch (err) {
...
async function saveHourlyEntry(slotId) {
  ...
  try {
    const res = await API.request('/api/batch/hourly', {
      method: 'POST',
      body: JSON.stringify({
        date: selectedDate,
        slotId: slotId,
    ...
    showToast('Batch hourly production saved successfully.', 'success');
    currentData = res.data;
    renderBatchDashboard();
  } catch (err) {
...
async function resetBatchData() {
  if (!confirm(`Are you sure you want to reset all Batch production data to 0 for ${selectedDate}?`)) return;

  try {
    const res = await API.request('/api/batch/reset', {
      method: 'POST',
      body: JSON.stringify({ date: selectedDate })
    });
    showToast(`Batch production data reset to 0 for ${selectedDate}`, 'success');
    currentData = res.data;
    renderBatchDashboard();
  } catch (err) {
    showToast(err.message || 'Failed to reset batch data', 'error');
  }
}

function triggerExport(format) {
  const url = `/api/export/download?module=batch&startDate=${selectedDate}&endDate=${selectedDate}&format=${format}`;
