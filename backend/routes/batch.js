...
router.get('/data', authenticateToken, requireRole(...ALLOWED_ROLES), async (req, res) => {
  try {
    ...
    console.error('Error getting batch data:', err);
    res.status(500).json({ error: 'Unable to retrieve batch production data.' });
  }
});

// Set daily target
router.post('/target', authenticateToken, requireRole('batch_supervisor', 'admin'), async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Batch daily target set successfully.', data: saved });
  } catch (err) {
    console.error('Error saving batch target:', err);
    res.status(500).json({ error: 'Unable to save daily target. Please try again.' });
  }
});

// Submit/update hourly achieved entry
router.post('/hourly', authenticateToken, requireRole('batch_supervisor', 'admin'), async (req, res) => {
  try {
    ...
    const saved = await dbService.setDocument(COLLECTION_NAME, docId, updatedDoc, true);
    res.json({ success: true, message: 'Batch hourly production saved successfully.', data: saved });
  } catch (err) {
    console.error('Error saving batch hourly production:', err);
    res.status(500).json({ error: 'Unable to save production data. Please try again.' });
  }
});

// Reset batch data for a date back to 0
router.post('/reset', authenticateToken, requireRole('batch_supervisor', 'admin'), async (req, res) => {
  try {
    const { date } = req.body;
    if (!date) {
      return res.status(400).json({ error: 'Date is required.' });
    ...
    res.json({ success: true, message: `Batch production data for ${date} has been reset to 0.`, data: saved });
  } catch (err) {
    console.error('Error resetting batch data:', err);
    res.status(500).json({ error: 'Unable to reset batch production data.' });
  }
});
...
router.get('/history', authenticateToken, requireRole(...ALLOWED_ROLES), async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Unable to fetch batch history.' });
  }
});
